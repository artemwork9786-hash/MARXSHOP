            ...
            <div className="mx-1.5 mb-1.5 rounded-xl bg-black/40 border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.7)]">
              <div className="flex items-center gap-1.5 px-2 py-1">
                ...
                <div className="relative shrink-0 flex items-center">
                  ...
                  <div className={`absolute left-1/2 -translate-x-1/2 bottom-full mb-4 z-50 transition-opacity duration-200 ${showVolume ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`} onMouseEnter={() => { clearTimeout(volumeTimer.current); setShowVolume(true); }} onMouseLeave={() => { if (!volumeDragging) volumeTimer.current = setTimeout(() => setShowVolume(false), 300); }}>
