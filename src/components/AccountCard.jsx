              ...
              <div className={`mx-1.5 mb-1.5 rounded-xl border border-white/[0.08] bg-black/63 backdrop-blur-md transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}>
                <div className="flex items-center gap-1.5 px-2 py-1">
                  ...
                  {volumeBtnRect && createPortal(
                    <div className="fixed z-[9999]" style={{ left: volumeBtnRect.left, top: volumeBtnRect.top, clipPath: showVolume ? "inset(0)" : "inset(0 0 100% 0)", transition: "clip-path 0.3s ease, transform 0.3s cubic-bezier(0.4,0,0.2,1)", transform: showVolume ? "translateY(0) scale(1)" : "translateY(12px) scale(0.95)", pointerEvents: showVolume ? "auto" : "none" }} onMouseEnter={() => { clearTimeout(volumeTimer.current); setShowVolume(true); }} onMouseLeave={() => { if (!volumeDragging) volumeTimer.current = setTimeout(() => setShowVolume(false), 300); }}>
