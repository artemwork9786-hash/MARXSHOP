              ...
              <div className={`mx-1.5 mb-1.5 rounded-xl border border-white/[0.08] bg-black/63 backdrop-blur-md transition-opacity duration-300 ${showControls ? "opacity-100" : "opacity-0"}`}>
                <div className="flex items-center gap-1.5 px-2 py-1">
                  ...
                  <div className="relative shrink-0 flex items-center">
                    ...
                    <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-4 z-50 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] origin-bottom" style={{ opacity: showVolume ? 1 : 0, transform: showVolume ? "translateY(0) scale(1)" : "translateY(8px) scale(0.95)", pointerEvents: showVolume ? "auto" : "none" }} onMouseEnter={() => { clearTimeout(volumeTimer.current); setShowVolume(true); }} onMouseLeave={() => { if (!volumeDragging) volumeTimer.current = setTimeout(() => setShowVolume(false), 300); }}>
                      <div className="flex flex-col items-center p-1.5 rounded-lg border border-white/[0.08] bg-black/63 backdrop-blur-md origin-bottom overflow-hidden shadow-[0_8px_24px_rgba(0,0,0,0.6)]">
